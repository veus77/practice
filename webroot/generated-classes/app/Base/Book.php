...
use app\Publisher as ChildPublisher;
use app\PublisherQuery as ChildPublisherQuery;
...
abstract class Book implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\app\\Map\\BookTableMap';
    ...
    /**
     * Initializes internal state of app\Base\Book object.
     */
     ...
     * @return $this|\app\Book The current object (for fluent API support)
     */
    ...
    public function setLastName($v)
    {
        ...
        if ($this->last_name !== $v) {
            $this->last_name = $v;
            $this->modifiedColumns[BookTableMap::COL_LAST_NAME] = true;
        }
    ...
    public function setAge($v)
    {
        ...
        if ($this->age !== $v) {
            $this->age = $v;
            $this->modifiedColumns[BookTableMap::COL_AGE] = true;
        }
    ...
    public function setDeveloper($v)
    {
        ...
        if ($this->developer !== $v) {
            $this->developer = $v;
            $this->modifiedColumns[BookTableMap::COL_DEVELOPER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : BookTableMap::translateFieldName('LastName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->last_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : BookTableMap::translateFieldName('Age', TableMap::TYPE_PHPNAME, $indexType)];
            $this->age = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : BookTableMap::translateFieldName('Developer', TableMap::TYPE_PHPNAME, $indexType)];
            $this->developer = (null !== $col) ? (int) $col : null;
            $this->resetModified();
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\app\\Book'), 0, $e);
        }
    }
    ...
    {
        if ($this->aPublisher !== null && $this->id !== $this->aPublisher->getId()) {
            $this->aPublisher = null;
        }
        if ($this->aCar !== null && $this->id !== $this->aCar->getId()) {
            $this->aCar = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(BookTableMap::COL_LAST_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'last_name';
        }
        if ($this->isColumnModified(BookTableMap::COL_AGE)) {
            $modifiedColumns[':p' . $index++]  = 'age';
        }
        if ($this->isColumnModified(BookTableMap::COL_DEVELOPER)) {
            $modifiedColumns[':p' . $index++]  = 'developer';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'age':
                        ...
                        break;
                    case 'developer':
                        $stmt->bindValue($identifier, $this->developer, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->aCar) {
                ...
                $result[$key] = $this->aCar->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\app\Book The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(BookTableMap::COL_LAST_NAME)) {
            $criteria->add(BookTableMap::COL_LAST_NAME, $this->last_name);
        }
        if ($this->isColumnModified(BookTableMap::COL_AGE)) {
            $criteria->add(BookTableMap::COL_AGE, $this->age);
        }
        if ($this->isColumnModified(BookTableMap::COL_DEVELOPER)) {
            $criteria->add(BookTableMap::COL_DEVELOPER, $this->developer);
        }
    ...
    public function hashCode()
    {
        ...
        //relation book_fk_566cdd to table publisher
        if ($this->aPublisher && $hash = spl_object_hash($this->aPublisher)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }

        //relation book_fk_4e3873 to table car
        if ($this->aCar && $hash = spl_object_hash($this->aCar)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \app\Book (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \app\Book Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aPublisher === null && ($this->id != 0)) {
            $this->aPublisher = ChildPublisherQuery::create()->findPk($this->id, $con);
            // Because this foreign key represents a one-to-one relationship, we will create a bi-directional association.
            $this->aPublisher->setBook($this);
        }
    ...
    /**
     * Declares an association between this object and a ChildCar object.
     *
     * @param  ChildCar $v
     * @return $this|\app\Book The current object (for fluent API support)
     * @throws PropelException
     */
    public function setCar(ChildCar $v = null)
    {
        if ($v === null) {
            $this->setId(NULL);
        } else {
    ...
    /**
     * Get the associated ChildCar object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildCar The associated ChildCar object.
     * @throws PropelException
     */
    public function getCar(ConnectionInterface $con = null)
    {
        if ($this->aCar === null && ($this->id != 0)) {
            $this->aCar = ChildCarQuery::create()->findPk($this->id, $con);
            // Because this foreign key represents a one-to-one relationship, we will create a bi-directional association.
            $this->aCar->setBook($this);
        }
